...
function UserEdit({ exit }) {
  ...
  useEffect(() => {
    axios
      .post(`${process.env.NEXT_PUBLIC_siteUrl}/api/userlist`)
      .then((res) => {
  ...
  useEffect(() => {
    if (edit.bool === false) {
      axios
        .post(`${process.env.NEXT_PUBLIC_siteUrl}/api/adminupdate`, edit)
        .then((res) => {
  ...
  const userDelete = () => {
    const deleteUser = () => {
      axios
        .post(`${process.env.NEXT_PUBLIC_siteUrl}/api/userdelete`, edit.i)
        .then((res) => {
          toast(res.data.message);
          if (typeof window !== "undefined") {
            location.reload();
