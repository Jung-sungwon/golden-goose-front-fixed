...
function Comment({ postId }) {
  ...
  const clickHandler = (e) => {
    if (user.email !== "") {
      axios.post(`${process.env.NEXT_PUBLIC_siteUrl}/api/comment`, {
        comData: comment,
        id: postId,
        name: user.name,
        email: user.email,
  ...
  useEffect(() => {
    axios
      .post(`${process.env.NEXT_PUBLIC_siteUrl}/api/loadcomment`, {
        id: postId,
      })
      .then((res) => {
        setComList(res.data);
  ...
  return (
    <div className={style.commentBoard}>
      <section className={style.comSec}>
        {comList.map((i, index) => {
          return (
            <div className={style.comDetailBoard} key={index}>
